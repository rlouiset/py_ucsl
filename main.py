    ...
    def update_S(self, X, y, S, index, cluster_index, idx_outside_polytope, lambda_sigmoid=5) :
        ...
        if self.clustering_strategy == 'k_means' :
            ...
            for cluster_i in range(self.n_clusters_per_label[idx_outside_polytope]):
                ...
                X_proj += 0.5 * X_proj_i
            '''
            directions = [self.coefficients[idx_outside_polytope][cluster_i] for cluster_i in range(self.n_clusters_per_label[idx_outside_polytope])]
            basis = []
            for v in directions:
            ...
            KM = KMeans(n_clusters=2, init=np.array(centroids), n_init=1).fit(X_proj[index])  #
