...
class HYDRA(BaseML):
    ...
    def update_S(self, X, y, S, index, cluster_index, idx_outside_polytope) :
        ...
        if self.clustering_strategy == 'custom':
            ...
            print(self.coefficients[idx_outside_polytope][0].shape)
            print(self.intercepts[idx_outside_polytope][0].shape)
            mean_b = np.mean([self.coefficients[idx_outside_polytope][cluster_i] for cluster_i in self.n_clusters_per_label], 0)
            mean_w = np.mean([self.intercepts[idx_outside_polytope][cluster_i] for cluster_i in self.n_clusters_per_label], 0)
            print(mean_w.shape)
            print(mean_b.shape)
            ...
            for cluster_i in range(self.n_clusters_per_label[idx_outside_polytope]) :
                ...
                boundary_barycenter_i = cluster_barycenters[cluster_i] + (cluster_barycenters[cluster_i]@mean_w.T+mean_b)*mean_w_norm
                print(boundary_barycenter_i.shape)
