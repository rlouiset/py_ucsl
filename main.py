...
class HYDRA(BaseML):
    ...
    def run(self, X, y, idx_outside_polytope):
        ...
        for consensus_i in range(n_consensus):
            ...
            for iter in range(self.n_iterations):
                ...
                self.S_momentum[idx_outside_polytope] = (0.9*S+0.1*self.S_momentum[idx_outside_polytope])
    ...
    def update_S(self, X, y, S, index, cluster_index, idx_outside_polytope, lambda_sigmoid=5) :
        ...
        if self.clustering_strategy == 'k_means' :
            ...
            GMM = GaussianMixture(n_components=2, init_params=np.array(centroids), covariance_type='diag').fit(X_proj[index])
