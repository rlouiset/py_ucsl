...
class HYDRA(BaseML):
    ...
    def run(self, X, y, idx_outside_polytope):
        ...
        for consensus_i in range(n_consensus):
            ...
            consensus_intercepts.append([self.intercepts[idx_outside_polytope][cluster_i][0] for cluster_i in range(len(self.intercepts[idx_outside_polytope]))])
    ...
    def apply_consensus(self, X, y_polytope, consensus_assignment, consensus_direction, consensus_intercepts, n_clusters, index_positives,
        ...
        if self.consensus == 'original':
            ...
            print(np.mean(mean_X, 0)*self.mean_direction[idx_outside_polytope])
            print(self.mean_intercept)
